...
> 해당 환경 특성상 일정 시간 이상 요청이 없으면 인스턴스가 자동으로 슬립 상태로 전환됩니다.  
> 따라서 처음 접속하거나 장시간 사용하지 않은 뒤 다시 요청할 경우, 애플리케이션이 재기동되며 최초 로딩에 약 3분 정도 시간이 소요될 수 있습니다.  
> 초기 구동이 완료된 후에는 정상적으로 이용할 수 있으며, 만약 화면이 정상적으로 표시되지 않는다면 3분 후 새로고침(F5)하여 다시 접속해 주세요.  
